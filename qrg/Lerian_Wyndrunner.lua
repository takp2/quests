...
function event_say(e)
	if(e.message:findi("hail")) then
		if(e.other:GetFactionValue(e.self) >= 750) then
			e.self:Emote("gives a tilt of his head toward " .. e.other:GetCleanName() .. ". 'Greetings, I am not native to htese lands, so I ask that you forgive me if I do not recognize your face, my friend. If [you have come to inquire about my presence here], then please, do rest assured that I am not a threat to these acred grounds. As Jaggedpine is from where I hail, know that my respect for this place is equal to that of any whom call this place home.");
		elseif(e.other:GetFactionValue(e.self) >= 0) then
			e.self:Emote("gives a slight smile and gentle nod of his head to you, 'Good day, my friend. I see The Rainkeeper has his grace reaching toward you -- the taint of corruption is blessedly absent from your being. However, you are a stranger yet to the Residents of the Pine. Go out into the world, my friend, and make your presence and deeds known as amiable to our cause and perhaps I may be able to be of service. May Karana guide and keep you well, my friend.");
		else
			e.self:Emote("raises his chin high and continues an emotionless study of " .. e.other:GetCleanName() .. ", 'You are tainted -- in spirit or in deeds I am unsure. However, your taint is apparent and I cannot be bothered with one as yourself. If and when the touch of corruption has abandoned you in the eyes of The Rainkeeper's faithful, you may return to me and I may be a bit more receptive to your company. Until that time arrives, traveler, I must ask that you move along.");
		end
...
function event_trade(e)
	...
	if(e.other:GetFactionValue(e.self) >= 750 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8766,item2 = 8767,item3 = 8765},0)) then -- flawless panther hide, sap, & dew of the hatchling
		...
		e.other:QuestReward(e.self,0,0,0,0,8764,2000); -- cured panther hide
	elseif(e.other:GetFactionValue(e.self) >= 750 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8799,item2 = 8797},0)) then -- shark rib & flayed panther hide
		...
		e.other:Faction(e.self,302,2);  -- Protectors of Pine
		e.other:Faction(e.self,262,1);  -- Guards of Qeynos
		e.other:QuestReward(e.self,0,0,0,0,8863,2000); --reinforced rawhide sleeves
	elseif(e.other:GetFactionValue(e.self) >= 750 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8879,item2 = 8873,item3 = 8878,item4 = 8878},0)) then -- jaggedpine panther hide,shadowjade fern seeds,2x shadowjade fern leaves
		e.self:Emote("studies the hide carefully -- using his eyes, fingertips, and sense of smell to be sure that it is indeed of the panthers native to the Jaggedpine forest. Accepting the true identity of the hide, Lerian removes a small vial of clear oil and smears it across the flawless midnight fur. He then retrieves the seeds from the small pouch given to him by " .. e.other:GetCleanName() .. ", and begins to chant lowly and softly in his ancient druidic tongue. The seeds begin to sprout, growing at an unnatural pace from the nourishing oil. The fern imbeds itself in a beautiful design upon the hide as they grow -- their roots and the thin, delicate vines rendering the flora secure in its place. Lerian then offers the hide to " .. e.other:GetCleanName() .. ", 'You will need to seek out one other item -- the Blackened Mithril Boots from the kobold priest in Solusek's eye. Once you have the boots, you will need a master's knowledge of tailoring to carefully fit this hide over the boots before returning them to Cheyloh.");
		e.other:Faction(e.self,1597,5);  -- Residents of Jaggedpine
		e.other:Faction(e.self,272,2);  -- Jaggedpine Treefolk
		e.other:Faction(e.self,302,2);  -- Protectors of Pine
		e.other:Faction(e.self,262,1);  -- Guards of Qeynos
		e.other:QuestReward(e.self,0,0,0,0,8881,2000); -- shadowjade hide
	elseif(e.other:GetFactionValue(e.self) >= 750 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8866,item2 = 8867},0)) then -- braids of pine & black rawhide bracer
		e.self:Emote("gently places the braids of pine across the rawhide bracer.  He murmurs a few soft syllables, and the braids begin to move and grow, as if of their own volition.  Wrapping around and through the blackened rawhide, the bracer is soon completely covered by the braids.  With twinkle in his eye, Lerian hands the bracer to " .. e.other:GetCleanName() .. ".");
		e.other:Faction(e.self,1597,5);  -- Residents of Jaggedpine
		e.other:Faction(e.self,272,2);  -- Jaggedpine Treefolk
		e.other:Faction(e.self,302,2);  -- Protectors of Pine
		e.other:Faction(e.self,262,1);  -- Guards of Qeynos
		e.other:QuestReward(e.self,0,0,0,0,8865,2000); -- bracer of braided pine 
	elseif(e.other:GetFactionValue(e.self) >= 750 and item_lib.check_turn_in(e.self, e.trade, {item1 = 8872,item2 = 8792,item3 = 8878},0)) then -- blackened mithril gloves & shadowjade moss & fern leaves
		e.self:Emote("examines the gloves, checking the item's stability and quality of make. Satisfied, Lerian then pulls the gloves onto his hands and proceeds to carefully cover the gloves with the dark emerald moss. He then closes his eyes and begins to whisper a soft chant in an ancient druidic language as he opens the small pouch and sprinkles the fern seeds across the palms and back of his hands. As he chants, his hands hold a soft emerald radiance and the seeds begin to sprout before your eyes. The tiny ferns weave in an out of the Mithril netting, binding the moss to the gloves and creating a beautiful design upon the back of each gloves. Lerian removes the gloves as his chant ends and extends them to " .. e.other:GetCleanName() .. ", a delicate, warm smile upon his face in silent approval.");
		e.other:Faction(e.self,1597,5);  -- Residents of Jaggedpine
		e.other:Faction(e.self,272,2);  -- Jaggedpine Treefolk
		e.other:Faction(e.self,302,2);  -- Protectors of Pine
		e.other:Faction(e.self,262,1);  -- Guards of Qeynos
		e.other:QuestReward(e.self,0,0,0,0,8871,2000); -- forest woven gloves
	end
